...
DATA_FILE="${GENIE3_LANDSCAPE_DATA_FILE:-${DATA_ROOT}/GC_landscape.json}"
OUT_ROOT="${GENIE3_OUT_ROOT:-${ROOT}/final-result/Landscape-GC}"
RUN_ID="${GENIE3_RUN_ID:-landscape_gc_$(date '+%Y%m%d_%H%M%S')}"
...
LOG_ROOT="${GENIE3_RUN_LOG_ROOT:-${ROOT}/final-result/Landscape-GC-logs}"
RUN_LOG_DIR="${LOG_ROOT}/${RUN_ID}"
PYTHON_BIN="${GENIE3_PYTHON_BIN:-python3}"
TOTAL_TASKS="$("${PYTHON_BIN}" -c 'import json,sys; print(len(json.load(open(sys.argv[1], encoding="utf-8"))))' "${DATA_FILE}")"
MAX_ROUNDS="${GENIE3_MAX_LANDSCAPE_ROUNDS:-20}"

mkdir -p "${OUT_ROOT}" "${RUN_LOG_DIR}"

export GENIE3_CDP_URL="${GENIE3_CDP_URL:-http://127.0.0.1:9229}"
...
export GENIE3_DATA_FILES="GC:${DATA_FILE}"
export GENIE3_ALL_TASKS=1
unset TASK_IDS PERSPECTIVES
...
export GENIE3_LIVE_HOLD_TIMEOUT_S="${GENIE3_LIVE_HOLD_TIMEOUT_S:-15}"
export GENIE3_ROTATION_LOOP_CHECK="${GENIE3_ROTATION_LOOP_CHECK:-1}"
export GENIE3_ROTATION_BASE_EXTRA_UNITS="${GENIE3_ROTATION_BASE_EXTRA_UNITS:-1}"
export GENIE3_ROTATION_LOOP_MAX_ACTIONS="${GENIE3_ROTATION_LOOP_MAX_ACTIONS:-6}"
export GENIE3_ROTATION_LOOP_MAX_CHECKS="${GENIE3_ROTATION_LOOP_MAX_CHECKS:-3}"
export GENIE3_FINAL_ORIENTATION_CHECK="${GENIE3_FINAL_ORIENTATION_CHECK:-1}"
export GENIE3_FINAL_ORIENTATION_MAX_ACTIONS="${GENIE3_FINAL_ORIENTATION_MAX_ACTIONS:-6}"
export GENIE3_FINAL_ORIENTATION_MAX_CHECKS="${GENIE3_FINAL_ORIENTATION_MAX_CHECKS:-2}"
...
valid_video_count() {
  local count=0
  local directory
  for directory in "${OUT_ROOT}"/LS???_"${RUN_TIMESTAMP}"; do
    [[ -d "${directory}" ]] || continue
    if find "${directory}" -maxdepth 1 -type f -name '*_native.*' -size +1000c | read -r _ \
      && grep -Eq '"status"[[:space:]]*:[[:space:]]*"(downloaded|completed)"' "${directory}/result.json"; then
      count=$((count + 1))
    fi
  done
  echo "${count}"
