...
DATA_FILE="${GENIE3_OE_DATA_FILE:-${DATA_ROOT}/OE.json}"
OUT_ROOT="${GENIE3_OUT_ROOT:-${ROOT}/result/genie3}"
RUN_ID="${GENIE3_RUN_ID:-oe_all_$(date '+%Y%m%d_%H%M%S')}"
...
MAX_ROUNDS="${GENIE3_MAX_OE_ROUNDS:-20}"
...
export GENIE3_DATA_FILES="OE:${DATA_FILE}"
...
valid_video_count() {
  local count=0
  local directory
  for directory in "${OUT_ROOT}"/OE???_"${RUN_TIMESTAMP}"; do
